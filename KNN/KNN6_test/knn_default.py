...
#8e - Cluster-quality metrics: how well the KMeans neighbourhoods (section 3.6)
#align with the true fault/no_fault labels, computed separately for validation
#and test so any drop in that alignment is visible per split rather than only
#as a single whole-dataset number.
CLUSTER_METRIC_FUNCS = {
...
neighbourhood_clustering_metrics = pd.DataFrame({
    split_name: {
        metric_name: metric_fn(dataset.loc[mask, "faulty"], dataset_clusters[mask])
        for metric_name, metric_fn in CLUSTER_METRIC_FUNCS.items()
    }
    for split_name, mask in (("validation", validation_mask), ("test", test_mask))
}).T

print(f"[neighbourhood comparison] neighbourhood-vs-fault clustering metrics by split:\n{neighbourhood_clustering_metrics}")
neighbourhood_clustering_metrics.to_csv(out_path("neighbourhood_clustering_metrics_validation_test.csv"))

fig, ax = plt.subplots()
x = np.arange(len(CLUSTER_METRIC_FUNCS))
width = 0.35
for offset, split_name in enumerate(neighbourhood_clustering_metrics.index):
    ax.bar(x + (offset - 0.5) * width, neighbourhood_clustering_metrics.loc[split_name], width, label=split_name.capitalize())
ax.set_xticks(x)
ax.set_xticklabels(list(CLUSTER_METRIC_FUNCS.keys()))
ax.set_ylabel("Score")
ax.set_title("Neighbourhood-vs-Fault Clustering Metrics: Validation vs Test")
ax.legend()
fig.tight_layout()
fig.savefig(out_path("neighbourhood_clustering_metrics_validation_test.png"))
plt.close(fig)

#8f - Per-neighbourhood classification metrics: how well the KNN classifier
#does specifically within each KMeans neighbourhood, for validation and test.
#Complements 8c (per-point neighbour purity) and 8e (whether neighbourhoods
#correlate with fault labels) with a direct accuracy/precision/recall/f1
#breakdown per neighbourhood, so a poorly performing split can be traced to
#the specific neighbourhood(s) responsible rather than only a whole-split number.
per_neighbourhood_split_masks = {"validation": validation_mask, "test": test_mask}
per_neighbourhood_rows = []
for method_label, split_predictions in predictions.items():
    for split_name, split_mask in per_neighbourhood_split_masks.items():
        y_true_arr = fault[split_mask].to_numpy()
        y_pred_arr = np.asarray(split_predictions[split_name])
        clusters_arr = dataset_clusters[split_mask].to_numpy()
        for cluster_id in range(k):
            cluster_mask = clusters_arr == cluster_id
            support = int(cluster_mask.sum())
            if support == 0:
                continue
            y_true_sub = y_true_arr[cluster_mask]
            y_pred_sub = y_pred_arr[cluster_mask]
            per_neighbourhood_rows.append({
                "method": method_label, "split": split_name, "neighbourhood": cluster_id,
                ...
                "f1-score": f1_score(y_true_sub, y_pred_sub, zero_division=0),
                "homogeneity": homogeneity_score(y_true_sub, y_pred_sub),
                "mutual_info": mutual_info_score(y_true_sub, y_pred_sub),
                "v_measure": v_measure_score(y_true_sub, y_pred_sub),
                "rand_index": rand_score(y_true_sub, y_pred_sub),
                "support": support,
            })

per_neighbourhood_metrics = pd.DataFrame(per_neighbourhood_rows).set_index(["method", "split", "neighbourhood"]).sort_index()
print(f"[neighbourhood comparison] per-neighbourhood classification metrics:\n{per_neighbourhood_metrics}")
per_neighbourhood_metrics.to_csv(out_path("neighbourhood_classification_metrics_validation_test.csv"))

per_neighbourhood_heatmap_cols = ["accuracy", "precision", "recall", "f1-score",
                                   "homogeneity", "mutual_info", "v_measure", "rand_index"]
fig, ax = plt.subplots(figsize=(1.4 * len(per_neighbourhood_heatmap_cols), max(4, 0.35 * len(per_neighbourhood_metrics))))
sns.heatmap(per_neighbourhood_metrics[per_neighbourhood_heatmap_cols], annot=True, fmt=".2f", cmap="Blues", ax=ax)
ax.set_title("Classification & Clustering Metrics per Neighbourhood (Validation vs Test)")
fig.tight_layout()
fig.savefig(out_path("neighbourhood_classification_metrics_validation_test.png"))
plt.close(fig)
