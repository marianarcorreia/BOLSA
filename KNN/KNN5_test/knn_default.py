...
#1 -Load the dataset
DATASET_PATH = Path(args.dataset) if args.dataset is not None else Path("C:\\Users\\maria\\OneDrive\\Desktop\\bolsa1\\KNN\\KNN5_test\\datasets\\stuck_PRESSURE_appropriate_dataset_all_runs.json")
...
#the manual implementation is brute-force (cdist against every reference
#point, for every query point, on every split incl. train-vs-train), so on a
#320k-row training split it blows up to hours of runtime and multiple GB of
#RAM; sklearn's KNN uses a tree and doesn't have this problem, so only the
#manual model's reference set is capped
MANUAL_KNN_MAX_TRAIN_ROWS = 5000
if len(train_features) > MANUAL_KNN_MAX_TRAIN_ROWS:
    _manual_train_idx = train_features.sample(n=MANUAL_KNN_MAX_TRAIN_ROWS, random_state=42).index
    manual_train_features, manual_y_train = train_features.loc[_manual_train_idx], y_train.loc[_manual_train_idx]
else:
    manual_train_features, manual_y_train = train_features, y_train
manual_knn = ManualKNNClassifier(manual_train_features, manual_y_train, k)
