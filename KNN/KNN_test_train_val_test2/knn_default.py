...
def evaluate_predictions(method_label, split_label, y_true, y_pred):
    ...
    disp.figure_.savefig(out_path(f"Confusion_Matrix_{method_label}_{split_label}.png"))
...
#1 -Load the dataset
DATASET_PATH = Path("C:\\Users\\maria\\OneDrive\\Desktop\\bolsa1\\KNN\\KNN_test_train_val_test2\\datasets\\bias_TEMP_appropriate_dataset_all_runs.json")
with open(DATASET_PATH) as f:
    dataset = pd.DataFrame(json.load(f))

# every generated file (charts, reports, confusion matrices) is written under
# "[faulty_type]_results/[sensor]" (e.g. drift_results/o2), derived from the
...
plt.ylabel("Accuracy")
plt.savefig(out_path("accuracy_train_test_vs_neighbors.png"))
plt.close(fig)
...
#not-faulty points it actually contains (whole dataset, all splits combined).
#A neighbourhood that is almost entirely one class is "easy" for KNN - any
#point landing there is surrounded by same-label neighbours. A neighbourhood
#close to a 50/50 split is where misclassifications are more likely, since a
#query point's k nearest neighbours will disagree on the label.
neighbourhood_fault_counts = pd.crosstab(dataset_clusters, dataset["faulty"])
neighbourhood_fault_counts = neighbourhood_fault_counts.rename(columns=FAULT_LABELS).reindex(range(k), fill_value=0)
for label in FAULT_LABELS.values():
    if label not in neighbourhood_fault_counts:
        neighbourhood_fault_counts[label] = 0
neighbourhood_fault_counts = neighbourhood_fault_counts[list(FAULT_LABELS.values())]
neighbourhood_fault_counts["total"] = neighbourhood_fault_counts.sum(axis=1)
neighbourhood_fault_proportions = neighbourhood_fault_counts[list(FAULT_LABELS.values())].div(
    neighbourhood_fault_counts["total"].replace(0, np.nan), axis=0
).fillna(0.0)

print(f"[neighbourhood comparison] fault vs not-faulty counts per neighbourhood:\n{neighbourhood_fault_counts}")
print(f"[neighbourhood comparison] fault vs not-faulty proportions per neighbourhood:\n{neighbourhood_fault_proportions}")
neighbourhood_fault_counts.to_csv(out_path("neighbourhood_fault_counts.csv"))
neighbourhood_fault_proportions.to_csv(out_path("neighbourhood_fault_proportions.csv"))

fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
x = np.arange(k)
for ax, data, ylabel, title in (
    (axes[0], neighbourhood_fault_counts, "Number of points", "Fault vs Not-faulty counts per neighbourhood"),
    (axes[1], neighbourhood_fault_proportions, "Proportion of neighbourhood's points", "Fault vs Not-faulty proportions per neighbourhood"),
):
    bottom = np.zeros(k)
    for fault_value, label in FAULT_LABELS.items():
        ax.bar(x, data[label], bottom=bottom, color=FAULT_COLORS[fault_value], label=label)
        bottom += data[label].to_numpy()
    ax.set_xticks(x)
    ax.set_xticklabels([str(c) for c in x], fontsize=8)
    ax.set_xlabel("Neighbourhood (KMeans cluster id)")
...
fig.suptitle("Fault Composition of Each KNN Neighbourhood (whole dataset)")
fig.tight_layout()
fig.savefig(out_path("neighbourhood_fault_composition.png"))
plt.close(fig)
